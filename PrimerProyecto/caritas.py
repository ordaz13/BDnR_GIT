...
edad = normaliza(edad)
genero = normaliza(genero)
hora = normaliza(hora)
distancia = normaliza(distancia)
viajes = normaliza(viajes)
#Grafica
fig = figure(figsize=(11,11))
for i in range(len(cp)):
    ax = fig.add_subplot(6,6,i+1,aspect='equal')
    cface(ax, 1, 1, 1, 1, viajes.values()[i], hora.values()[i], 1, distancia.values()[i], 1, 1, 1, 1, 1, edad.values()[i], 1, 1, genero.values()[i], 1)
